...
<template>
  <div class="group flex flex-col h-full bg-white animate-luxury-fade">
    <NuxtLink :to="`/products/${product.slug}`" class="relative aspect-[4/5] overflow-hidden bg-luxury-cream">
      ...
      <!-- Quick Badges -->
      <div v-if="product.salePrice" class="absolute top-2 left-2 bg-luxury-gold text-white text-[10px] font-bold px-2 py-1 uppercase">
        Offre Spéciale
      </div>
      
      <div v-if="product.stock < 5 && product.stock > 0" class="absolute bottom-2 left-2 bg-white/90 text-red-600 text-[10px] font-bold px-2 py-1 uppercase">
