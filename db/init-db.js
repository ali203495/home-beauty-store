...
const INITIAL_PRODUCTS = [
    ...
    { id: 'mk-02', name: 'Fond de Teint Velours Lumière', category: 'Maquillage', price: 380, rating: 4.8, reviews: 156, image: 'assets/webp/makeup.webp', badge: '', visible: 1, description: 'Base de maquillage à couvrance totale avec fini naturel and lumineux.', specs: JSON.stringify({ 'Volume': '30ml', 'SPF': '15', 'Type de peau': 'Tous types' }), stock: 25 },
    ...
    { id: 'kit-02', name: 'Service de Table en Céramique (18 pcs)', category: 'Cuisine', price: 850, rating: 4.9, reviews: 15, image: 'assets/webp/kitchen_cleaning.webp', badge: 'Premium', visible: 1, description: 'Élégant service de table complet pour 6 personnes, design moderne and épuré.', specs: JSON.stringify({ 'Pièces': '18', 'Compatibles': 'Micro-ondes/Lave-vaisselle' }), stock: 25 },
    { id: 'cln-01', name: 'Pack Nettoyage Éclat Total', category: 'Fournitures de Nettoyage', price: 250, rating: 4.6, reviews: 78, image: 'assets/webp/kitchen_cleaning.webp', badge: '', visible: 1, description: 'Kit complet comprenant nettoyant multi-surfaces, dégraissant and lustrant.', specs: JSON.stringify({ 'Volume Total': '3L', 'Parfums': 'Citron/Lavande' }), stock: 25 },
    { id: 'cln-02', name: 'Aspirateur Robot CleanPro X', category: 'Équipements de Nettoyage', price: 2400, rating: 4.7, reviews: 64, image: 'assets/webp/appliances.webp', badge: 'Promo', visible: 1, description: 'Robot aspirateur intelligent avec navigation laser and fonction lavage de sol.', specs: JSON.stringify({ 'Autonomie': '120 min', 'Aspiration': '2500 Pa', 'Bruit': '60 dB' }), stock: 25 },
...
async function initialize() {
    ...
    try {
        ...
        // Translation layer to make the same schema.sql work for both SQLite and Postgres
        let translatedSchema = schema;
        const isPostgres = process.env.POSTGRES_URL || process.env.DATABASE_URL;

        if (isPostgres) {
            // SQLite -> Postgres
            translatedSchema = translatedSchema
                .replace(/INTEGER PRIMARY KEY AUTOINCREMENT/g, 'SERIAL PRIMARY KEY')
                .replace(/DATETIME/g, 'TIMESTAMP')
                .replace(/BOOLEAN DEFAULT 1/g, 'BOOLEAN DEFAULT TRUE')
                .replace(/BOOLEAN DEFAULT 0/g, 'BOOLEAN DEFAULT FALSE');
        } else {
            // Postgres -> SQLite (in case someone edited schema.sql for Postgres)
            translatedSchema = translatedSchema
        ...
        await db.exec(translatedSchema);
        ...
        for (let p of INITIAL_PRODUCTS) {
            ...
            await db.run(query, [p.id, p.name, p.category, p.price, p.rating, p.reviews, p.image, p.badge, p.visible, p.description, p.specs, p.stock, new Date().toISOString()]);
        }

        const adminQuery = `INSERT INTO admins (username, passwordHash, role, status) VALUES (?, ?, ?, ?) ON CONFLICT (username) DO NOTHING`;
        await db.run(adminQuery, ['admin', '$2b$10$EpRnsD4GZp4WvL3pSInJ.O3662I1c88JvYv7q8J8Zf4a6E1D7u67y', 'Super Admin', 'Active']);
